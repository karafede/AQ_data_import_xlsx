...
[type,sheetname] = xlsfinfo('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\MoEW Data\NCMS data 2013.xlsx'); 
%%%% loop for the sheets
%tic
for k=1:length(sheetname) % minus one depending on the worksheet if it has a sheet named 'lists' at the end 
    if strcmp(sheetname{1,k}, 'Lists' ) || strcmp(sheetname{1,k}, 'lists' )
    else
    data = importfile3('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\MoEW Data\NCMS data 2013.xlsx',sheetname{1,k},startRow, endRow);
...
%toc
save('AQ_raw_data_NCMS_2013_hourly.mat', 'raw_data')
...
[type,sheetname] = xlsfinfo('Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\dawit Data\Stations_NCMS.xlsx'); 
...
for kk=1: length(sheetname)
    if strcmp(sheetname{1,kk}, 'Units' )
        ...
        polu_units = units('Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\dawit Data\Stations_NCMS.xlsx', sheetname{1,kk});
    else
        ...
        Station_detail = import_station_detail('Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\dawit Data\Stations_NCMS.xlsx',sheetname{1,kk},startRow,endRow);
...
save('AQ_arranged_data_NCMS_2013_hourly.mat', 'arra_dataset_station')
%
...
export(final_dataset,'File','Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\dawit Data\database_NCMS_2013_hourly_FK.csv','Delimiter',',')
clear all
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%       2014       %%%%%%%%%%%%%%%%%%%%
...
startRow = 10; % the row that the data starts
endRow = 8769; % 8769 or 8793 for the entire year 
[type,sheetname] = xlsfinfo('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\MoEW Data\NCMS data 2014.xlsx'); 
%%%% loop for the sheets
%tic
for k=1:length(sheetname)%-1 % minus one depending on the worksheet if it has a sheet named 'lists' at the end 
    if strcmp(sheetname{1,k}, 'Lists' ) || strcmp(sheetname{1,k}, 'lists' )
    else
    data = importfile3('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\MoEW Data\NCMS data 2014.xlsx',sheetname{1,k},startRow, endRow);
    dawit=sheetname{1,k}; % to create valid name with out spaces
    sheetname{1,k}=genvarname(dawit); %on the updated version matlab.lang.makeValidName can be used instead of genvarname
    raw_data.(sheetname{1,k}) = data; 
    end
...
%on the updated version matlab.lang.makeValidName can be used instead of genvarname
%toc
save('AQ_raw_data_NCMS_2014_hourly.mat', 'raw_data')
clearvars data sheetname type dawit startRow endRow k
%%%% end of the loop the result will be a structure with file name of raw_data
...
save('AQ_arranged_data_NCMS_2014_hourly.mat', 'arra_dataset_station')
%
...
clearvars -except final_dataset
%%%%%% Exporting the file to csv file
export(final_dataset,'File','Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\dawit Data\database_NCMS_2014_hourly.csv','Delimiter',',')
clear all
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%       2015       %%%%%%%%%%%%%%%%%%%%
...
startRow = 10; % the row that the data starts
endRow = 8769; % 8769 or 8793 for the entire year 
[type,sheetname] = xlsfinfo('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\MoEW Data\NCMS data 2015.xlsx'); 
%%%% loop for the sheets
%tic
for k=1:length(sheetname)%-1 % minus one depending on the worksheet if it has a sheet named 'lists' at the end 
    if strcmp(sheetname{1,k}, 'Lists' ) || strcmp(sheetname{1,k}, 'lists' )
    else
    data = importfile3('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\MoEW Data\NCMS data 2015.xlsx',sheetname{1,k},startRow, endRow);
    dawit=sheetname{1,k}; % to create valid name with out spaces
    sheetname{1,k}=genvarname(dawit); %on the updated version matlab.lang.makeValidName can be used instead of genvarname
    raw_data.(sheetname{1,k}) = data;  
    end
...
%on the updated version matlab.lang.makeValidName can be used instead of genvarname
%toc
save('AQ_raw_data_NCMS_2015_hourly.mat', 'raw_data')
clearvars data sheetname type dawit startRow endRow k
%%%% end of the loop the result will be a structure with file name of raw_data
...
save('AQ_arranged_data_NCMS_2015_hourly.mat', 'arra_dataset_station')
%
...
clearvars -except final_dataset
%%%%%% Exporting the file to csv file
export(final_dataset,'File','Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\dawit Data\database_NCMS_2015_hourly.csv','Delimiter',',')
clear all
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%       2016       %%%%%%%%%%%%%%%%%%%%
...
[type,sheetname] = xlsfinfo('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\MoEW Data\NCMS data 2016.xlsx'); 
%%%% loop for the sheets
%tic
for k=1:length(sheetname)%-1 % minus one depending on the worksheet if it has a sheet named 'lists' at the end 
    if strcmp(sheetname{1,k}, 'Lists' ) || strcmp(sheetname{1,k}, 'lists' )
    else
    data = importfile3('Z:\_SHARED_FOLDERS\Air Quality\Phase 2\MoEW Data\NCMS data 2016.xlsx',sheetname{1,k},startRow, endRow);
    dawit=sheetname{1,k}; % to create valid name with out spaces
    sheetname{1,k}=genvarname(dawit); %on the updated version matlab.lang.makeValidName can be used instead of genvarname
    raw_data.(sheetname{1,k}) = data;  
    end
...
%on the updated version matlab.lang.makeValidName can be used instead of genvarname
%toc
save('AQ_raw_data_NCMS_2016_hourly.mat', 'raw_data')
clearvars data sheetname type dawit startRow endRow k
%%%% end of the loop the result will be a structure with file name of raw_data
...
save('AQ_arranged_data_NCMS_2016_hourly.mat', 'arra_dataset_station')
%
...
clearvars -except final_dataset
%%%%%% Exporting the file to csv file
% export(final_dataset,'File','Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\dawit Data\database_NCMS_2016_hourly.csv','Delimiter',',')
export(final_dataset,'File','D:\procedures for AQ data processing\1_importing from excel\database_NCMS_2016_hourly.csv','Delimiter',',')
