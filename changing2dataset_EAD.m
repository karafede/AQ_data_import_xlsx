...
%%%%%% Data from the Emirate of Abu Dhabi 
startRow = 10; % the row that the data starts
endRow = 2169; % this is only 1st Quarter of 2015 from jan 1st to march 31st 
[type,sheetname] = xlsfinfo('Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\RAW DATA\MoEW Data\2015 data\wetransfer-43afdf\EAD  Data 1st Quar. 2015.xlsx'); 
%%%% loop for the sheets
%tic
for k=1:length(sheetname)-1 % minus one depending on the worksheet if it has a sheet named 'lists' at the end 
    data = importfile3('Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\RAW DATA\MoEW Data\2015 data\wetransfer-43afdf\EAD  Data 1st Quar. 2015.xlsx',sheetname{1,k},startRow, endRow);
...
%toc
save('AQ_raw_data_EAD_1_Quar_2015_hourly.mat', 'raw_data')
...
for ii=14:length (names) % a loop to all the stations in the structure
...
save('AQ_arranged_data_EAD_1_Quar_2015_hourly.mat', 'arra_dataset_station')
%
...
export(final_dataset,'File','Z:\_SHARED_FOLDERS\Air Quality\Phase 1\Pathflow of Phase I_DG\dawit Data\database_EAD_1_Quar_2015_hourly.csv','Delimiter',',')
